...
const seedDatabaseData = async () => {
    console.log('[Seed] Seeding database safely without wiping active users...');
    // Admin User
    ...
    // Business Settings
    const settingsCount = await BusinessSettings_1.BusinessSettings.countDocuments();
    if (settingsCount === 0) {
        await BusinessSettings_1.BusinessSettings.create({
    ...
    // Working Hours (10:30 AM to 9:30 PM)
    const hoursCount = await WorkingHours_1.WorkingHours.countDocuments();
    if (hoursCount === 0) {
    ...
    for (const s of servicesData) {
        try {
            ...
            let serv = await Service_1.Service.findOne({ slug });
            if (!serv) {
                serv = await Service_1.Service.create({ ...s, slug });
            }
    ...
    // Packages Catalog
    const packagesCount = await Package_1.Package.countDocuments();
    if (packagesCount === 0 && createdServices.length > 0) {
        try {
    ...
    // Staff Roster
    const staffCount = await Staff_1.Staff.countDocuments();
    if (staffCount === 0) {
        try {
    ...
    // Testimonials
    const testimonialCount = await Testimonial_1.Testimonial.countDocuments();
    ...
    // Gallery Portfolio
    const galleryCount = await Gallery_1.Gallery.countDocuments();
    if (galleryCount === 0) {
        try {
            await Gallery_1.Gallery.create([
...
const runStandaloneSeed = async () => {
    try {
        ...
        await (0, exports.seedDatabaseData)();
