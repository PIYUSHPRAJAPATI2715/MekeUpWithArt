...
exports.seedRealMenuData = void 0;
...
const seedRealMenuData = async () => {
    ...
    await Package_1.Package.deleteMany({});
    await WorkingHours_1.WorkingHours.deleteMany({});
    await Gallery_1.Gallery.deleteMany({});
    await Testimonial_1.Testimonial.deleteMany({});
    await Staff_1.Staff.deleteMany({});
    ...
    for (const s of realServices) {
        const slug = (0, slugify_1.slugify)(s.name);
        await Service_1.Service.create({
            name: s.name,
            slug,
            category: s.category,
    ...
    for (const p of realPackages) {
        const slug = (0, slugify_1.slugify)(p.name);
...
exports.seedRealMenuData = seedRealMenuData;
